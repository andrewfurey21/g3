...
function animate() {
    ...
    if (dist(hill.x, hill.y, player.x, player.y) < hill.r + player.w / 2) {
        player.points += .1;
    }
    hill.r = 100;
    hill.angle += hillAngleSpeed;
    hill.x = orbitR * Math.cos(hill.angle * Math.PI / 180);
    hill.y = orbitR * Math.sin(hill.angle * Math.PI / 180);
    ...
    if (pups.length >= 1) {
        for (let i = pups.length - 1; i >= 0; i--) {
            ...
            if (dist(pups[i].x, pups[i].y, player.x, player.y) < pups[i].r + player.w / 2) {
